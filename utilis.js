...
class SmartInput {
    ...
    initManual() {
        this.input.onfocus = () => { 
            if(!this.isManualOnly) this.input.value = this.input.value.replace(/[^\d.]/g, ''); 
        };
        this.input.onblur = () => { 
            if(this.input.value !== "") {
    ...
    setValue(val) {
        ...
        // If it's a standard numeric input (your HTML), don't add the unit string inside the box
    ...
    syncToMaster(val) {
        const num = parseFloat(val) || 0;
        if (typeof TAX_MASTER_ARRAY !== 'undefined' && this.masterIndex !== undefined) {
...
const TaxManager = {
    ...
    init() {
        // 1. COMPLEX INPUTS (Created by JS with Dropdowns)
        this.create('sea-pricePerCBM-input', 'Price per CBM (💲):', ' usd', [180, 150, 100, 50, 200], undefined, 'sea-pricePerCBM-container');
        this.create('sea-customsDutyRate-input', '🧾 Import Duty Rate:', '%', [10, 25, 35], 0, 'sea-tax-fields-main');
        this.create('sea-vatRate-input', '🧾 VAT Rate:', '%', [18, 15, 20], 1, 'sea-tax-fields-main');

        // 2. SIMPLE INPUTS (Bound to your existing HTML IDs)
        // No presets passed = No dropdown logic
    ...
    applyMasterArray(rates) {
        ...
        keys.forEach((id, i) => {
            if(this.instances[id]) this.instances[id].setValue(rates[i]);
        });
    }
};
